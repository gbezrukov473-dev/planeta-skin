...
npm run build        # full prod build → dist/ (prebuild: consistency checks + sitemap; postbuild: critical CSS via Critters)
npm run build:fast   # same, but skips critical CSS AND prebuild — use for quick local checks
npm run preview      # serve built dist/ at http://localhost:4173
npm run check        # pageMap ↔ HTML consistency + catalog.php ↔ products.js sync (also runs in prebuild)
...
- Each page contains placeholder comments: `<!-- HEAD_PLACEHOLDER -->`, `<!-- HEADER_PLACEHOLDER -->`, `<!-- FOOTER_PLACEHOLDER -->`, and (where present) `<!-- LEAD_FORM_PLACEHOLDER -->`.
- The plugin substitutes them with `templates/head.html`, `templates/header.html`, `templates/footer.html`, and a rendered `templates/lead-form.html`.
- `templates/head.html` holds the boilerplate `<head>` tags shared by every content page (favicons, manifest, `theme-color`, font preloads) — add a favicon or preload once here, not 16 times. Its HTML comments are stripped on insertion, so they don't ship. `404.html`, `thanks.html` and `policy.html` are deliberately standalone: they have their own tag sets and no `HEAD_PLACEHOLDER`. Per-page SEO (`title`, `description`, `canonical`, og/twitter, schema.org) stays in the page itself.
...
- **Core modules** are statically imported and initialized on `DOMContentLoaded` on every page: `preloader`, `mobileMenu`, `scrollToForm`, `forms` (lead form submit + masking + tracking), `scrollToTop`, `cookieBanner`, `analytics`, `lazyImages`, `phoneMask`.
- **Analytics is consent-gated.** Yandex.Metrika (`webvisor: true`, i.e. it records form input) and Top.Mail.Ru are no longer inline in each page's `<head>` — `src/js/modules/analytics.js` injects them only when `cookie_consent === 'accepted'`, either on load or via the `cookieBannerClosed` event. Counter IDs live in `src/js/config.js`. Don't put counter snippets back into HTML: that would make the consent banner decorative again and collect personal data without consent. A page can opt out entirely with `<meta name="pzk-analytics" content="off">` — `thanks.html` uses it, since it loads the bundle but is deliberately not tracked.
...
- Anti-spam: honeypot field `website`, `fill_time` heuristic, IP rate limiter (auto-cleaned after 1h).
- **Personal data lives OUTSIDE the webroot.** `data_dir()` resolves, in order: `data_dir` from `config.local.php` → `<parent of webroot>/planeta-data` → (last resort) the legacy `send-form/../data`. The legacy path is inside the webroot and on reg.ru nginx serves static files past Apache, so `.htaccess` there is not a reliable guard — that's why the directory was moved. Existing `*.jsonl` files are migrated out of the legacy dir automatically on first request. Verify with `curl -I https://hs-planet.ru/data/leads.jsonl` → expect 404.
- Leads append to `leads.jsonl` in that directory. On the first request of a new month the file is rotated to `leads-YYYY-MM.jsonl` automatically.
- **Captcha is fail-closed on prod.** If `config.local.php` (with `captcha.enabled` + `server_key`) is missing on `hs-planet.ru`, the form refuses submissions with a 503 and the clinic's phone number instead of silently accepting them unverified. The contact still lands in `drops.jsonl`. Emergency valve: `captcha.allow_missing = true`.
- Rejections split in two: unambiguous bot signals (`bot_ua`, `referer_foreign`, `honeypot`, `fill_time_fast`) get a fake success so bots can't probe; signals that may be a real person (empty Referer, no JS) get an honest error with the phone number. Both are logged to `drops.jsonl`.
- `ops-counters.json` (in the data dir) keeps 3 days of **mail failure** counts. When non-zero, a one-line summary is appended to the next lead email — the JSONL logs are nobody's daily reading, the inbox is. Rejected submissions used to be counted here too, but the email line pointed at `drops.jsonl`, which the admins reading those emails can't open — don't re-add a counter to the email unless whoever reads it can act on it.
...
- Don't add HTML pages without also registering them in `pageMap` (`vite-plugin-html-template.js`) — otherwise menu highlighting, lead-form id, and the service label sent to the backend will all be wrong. `scripts/check-pages.mjs` fails the build if you forget (it also catches duplicate `formId`/`formIdSuffix`), but only for pages that actually use the header/lead-form placeholders — `policy.html` deliberately has its own simplified header and no form, so it's not in `pageMap`.
